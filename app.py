...
# API Endpoint to fetch Netflix data
@app.route('/api/data')
def get_data():
    # Load Netflix data
    df = pd.read_excel('Data/netflix_titles.xlsx', sheet_name='description')
    imdb_df = pd.read_excel('Data/netflix_titles.xlsx', sheet_name='imdb_score')

    # Merge the data
    merged = pd.merge(df, imdb_df, on='title_id', how='inner')
    #print(merged)

    # Example data for graphs
    data = {
        "graph1": merged['release_year'].value_counts().to_dict(),  # Graph 1: Count by release year
        "graph2": merged['age_certification'].value_counts().to_dict(),  # Graph 2: Count by country
        "graph3": merged['runtime'].value_counts().to_dict(),  # Graph 3: IMDb stats
    }
